...
function testThing(message, generatePromise, timeout = 10) {
...
moduleFor('Adapter Errors: .then callback', testThing('errors in promise constructor', (error) => {
  new RSVP.Promise(() => {
    throw error;
  });
}));

moduleFor('Adapter Errors: Promise Contructor', testThing('errors in promise constructor', (error) => {
  RSVP.resolve().then(() => {
    throw error;
  });
}));

moduleFor('Adapter Errors: Promise chain .then callback', testThing('errors in promise constructor', (error) => {
